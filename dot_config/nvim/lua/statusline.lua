...
M.file_name = function(hl)
  ...
  if ft_overrides[ft] then
    return format_component(ft_overrides[ft].icon, ft_overrides[ft].icon_hl, ' ', '') .. format_component(ft_overrides[ft].name, hl)
  end
...
M.other_buffers = function(hl)
  local other_bufs = require('utils').get_buffer_count() - 1
